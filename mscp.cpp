...
static struct side white, black, *companion, *enemy;
...
/* Constants for static move ordering (pre-scores) ----------------------------------------ERRORS-unsigned type okay?*/
inline unsigned PRESCORE_EQUAL() {
	return(10U<<9);
	}
inline unsigned PRESCORE_HASHMOVE() {
	return(3U<<14);
	}
inline unsigned PRESCORE_KILLERMOVE() {
	return(2U<<14);
	}
inline unsigned PRESCORE_COUNTERMOVE() {
	return(1U<<14);
	}
...
static int test_illegal(int move)
{
        ...
        compute_attacks();
        unmake_move();
        return companion->attack[enemy->king] != 0;
}
...
check:
        make_move(move);
        compute_attacks();
        if (enemy->attack[companion->king]) {      /* in check, is mate? */
                int sign = '#';
                moves = move_sp;
                generate_moves(0);
                while (move_sp > moves) {
...
static int qsearch(int alpha, int beta)
{
        ...
        while (move_sp > moves) {
                ...
                compute_attacks();
                if (companion->attack[enemy->king]) {
                        unmake_move();
                        continue;
                }
...
static int search(int depth, int alpha, int beta)
{
        ...
        incheck = enemy->attack[companion->king];
...
static int root_search(int maxdepth)
{
        ...
        for (depth = 1; depth <= maxdepth; depth++) {
                ...
                while (m < move_sp) {
                        make_move(m->move);
                        compute_attacks();
                        if (companion->attack[enemy->king] != 0) { /* illegal? */
                                unmake_move();
